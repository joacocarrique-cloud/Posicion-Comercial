...
const ESC_CROPS = ['trigo', 'maiz', 'soja', 'girasol'];
const ESC_CROP_LBL = { trigo: 'Trigo', maiz: 'Maíz', soja: 'Soja', girasol: 'Girasol' };
// Columna del tablero que representa a cada cultivo (maíz = total temprano + tardío)
const ESC_COL = { trigo: 'TRIGO', maiz: 'MAIZ TOTAL', soja: 'SOJA', girasol: 'GIRASOL' };
const ESC_FILAS = {
...
const ESC_RANGO = 0.40;                        // ±40% en el gráfico y el slider
...
function escCompactar(d) {
  ...
  const cultivos = {};
  ESC_CROPS.forEach(c => {
    const i = D.cultivos.findIndex(x => escNorm(x.nombre) === ESC_COL[c]);
    if (i < 0) return;
    const o = {};
  ...
  const fyo = ((d.fyo || {}).posiciones || []).filter(p => Math.abs(p.neta) > 1e-9).map(p => {
    ...
    const crop = escNorm(p.cultivo).toLowerCase();
  ...
  }).filter(p => ESC_CROPS.includes(p.crop));

  return { meta: d.meta || {}, cultivos, fyo, importado: new Date().toISOString() };
}
...
function escImportar(ev) {
  ...
  rd.onload = () => {
    try {
      ...
      if (!escPos.cultivos[escState.crop]) escState.crop = Object.keys(escPos.cultivos)[0];
      escGuardar();
      escRender();
    } catch (e) {
...
function escFutA3(c, pos) {
  if (!sheetData || !pos || !sheetData.futuros[c]) return null;
  const f = sheetData.futuros[c].find(x => x.pos === pos && x.precio > 0);
...
function escFyo(c) { return escPos.fyo.filter(l => l.crop === c); }
function escLegs(c) { return escState.legs[c] || (escState.legs[c] = []); }
...
function escPrecioFinal(c, d, conProp) { return escIngreso(c, d, conProp) / escPos.cultivos[c].disp; }

// Cobertura a la baja con el criterio del tablero: (fijadas + futuros vendidos + puts netos) / producción
...
function escPosicionesA3(c) {
  if (!sheetData || !sheetData.futuros[c]) return [];
  return sheetData.futuros[c].filter(f => f.precio > 0).map(f => f.pos);
}
function escCadena(c, pos, tipo) {
  const byPos = sheetData && (sheetData.opciones[c] || {})[pos];
  if (!byPos) return [];
...
// Posición por defecto para cubrir: la de PRECIOS_REFERENCIA del cultivo si tiene puts, si no la de Coberturas
function escPosDefault(c) {
  ...
  const mesRef = Object.keys(typeof PRECIOS_REFERENCIA !== 'undefined' ? PRECIOS_REFERENCIA : {})
    .filter(k => k.startsWith(c + '|')).map(k => k.split('|')[1]);
  const ref = posA3.find(pos => mesRef.includes(pos.slice(0, 3)) && conPuts(pos));
  if (ref) return ref;
  const def = typeof defaultPositionFor === 'function' ? defaultPositionFor(c) : '';
...
function escRefrescarPrimas() {
  Object.keys(escState.legs).forEach(c => escLegs(c).forEach(l => {
...
function escLimpiar(soloCultivo) {
  if (soloCultivo) escState.legs[escState.crop] = [];
...
function escRender() {
  ...
  const m = escPos.meta || {};
  const cropsPos = ESC_CROPS.filter(c => escPos.cultivos[c]);
  ...
  body.innerHTML = `
    ...
    <div class="esc-tabs">${cropsPos.map(c => `<button class="esc-tab ${c === escState.crop ? 'active' : ''}" onclick="escSetCrop('${c}')">${ESC_CROP_LBL[c]}</button>`).join('')}</div>
    ...
    <div class="esc-grid">
      ...
      <div>
        ...
        <div class="rs-card"><div class="esc-chart-wrap"><canvas id="esc-chart"></canvas></div></div>
        <div class="rs-card">
...
function escHtmlPosicion() {
  ...
  const fyoRows = fyo.map(l => {
    ...
    return `<tr class="${escFyoIncluido(l) ? '' : 'esc-off'}">
      <td class="rs-l">${escHtml(l.ins)}</td><td>${lado}</td><td>${escF(l.tn, 0)}</td><td>${precio}</td>
      <td>${F0 > 0 ? escF(F0) : '—'}</td><td class="esc-c">${ctrl}</td></tr>`;
  }).join('');

  return `<h3>Posición ${ESC_CROP_LBL[c]}</h3>
...
function escHtmlPropuesta() {
  ...
  const p = escPos.cultivos[c];
  return `<h3>Cobertura propuesta ${ESC_CROP_LBL[c]}</h3>
...
function escRenderResultados() {
  ...
  if (lbl) lbl.textContent = `${escSig(d * 100, 0)}% · ${ESC_CROP_LBL[c]} a u$s ${escF(escMercado(c) * (1 + d))}`;
...
// Línea vertical en el escenario elegido
const escLineaEscenario = {
  id: 'escLinea',
  afterDatasetsDraw(ch, args, opts) {
    const v = opts && opts.value, x = ch.scales.x;
    if (!(v > 0) || v < x.min || v > x.max) return;
    const px = x.getPixelForValue(v), { top, bottom } = ch.chartArea, ctx = ch.ctx;
    ctx.save();
    ctx.strokeStyle = 'rgba(28,33,24,0.45)'; ctx.lineWidth = 1; ctx.setLineDash([4, 4]);
    ctx.beginPath(); ctx.moveTo(px, top); ctx.lineTo(px, bottom); ctx.stroke();
    ctx.setLineDash([]);
    const txt = opts.label || '';
    ctx.font = '600 11px Montserrat, sans-serif';
    const w = ctx.measureText(txt).width + 10;
    ctx.fillStyle = 'rgba(255,255,255,0.92)'; ctx.fillRect(px - w / 2, top + 4, w, 18);
    ctx.fillStyle = '#1c2118'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
...
function escRenderChart(c, d) {
  ...
  const hay = escLegs(c).length > 0;
  const act = [], prop = [], mkt = [];
  for (let k = -ESC_RANGO * 100; k <= ESC_RANGO * 100 + 1e-9; k++) {
    const dd = k / 100, x = M * (1 + dd);
    mkt.push({ x, y: x });
  ...
  [['Precio Objetivo', p.objetivo, typeof REF_OBJ_COLOR !== 'undefined' ? REF_OBJ_COLOR : '#7c3aed'],
   ['Precio Dolor', p.dolor, typeof REF_DOL_COLOR !== 'undefined' ? REF_DOL_COLOR : '#c43030']].forEach(([n, v, col]) => {
    if (!(v > 0)) return;
    refs.push(v);
    ds.push(linea(`${n} (${escF(v)})`, [{ x: M * (1 - ESC_RANGO), y: v }, { x: M * (1 + ESC_RANGO), y: v }], col,
      { borderWidth: 1.6, borderDash: [10, 5], pointHoverRadius: 0, order: 20 }));
  });
  const ys = act.concat(prop).map(o => o.y).concat(refs);
  const yMin = Math.floor(Math.min(...ys, M * (1 - ESC_RANGO) * 0.98) / 10) * 10;
  const yMax = Math.ceil(Math.max(...ys, M * (1 + ESC_RANGO) * 1.02) / 10) * 10;

  if (escChart) escChart.destroy();
  escChart = new Chart(canvas, {
    type: 'line',
    data: { datasets: ds },
    plugins: [escLineaEscenario],
    options: {
      responsive: true, maintainAspectRatio: false, animation: { duration: 0 }, parsing: false,
      interaction: { mode: 'index', intersect: false },
      plugins: {
        escLinea: { value: M * (1 + d), label: `Escenario ${escSig(d * 100, 0)}%` },
        tooltip: { callbacks: {
      ...
      scales: {
        x: { type: 'linear', min: M * (1 - ESC_RANGO), max: M * (1 + ESC_RANGO),
             title: { display: true, text: `Precio de mercado ${ESC_CROP_LBL[c]} a vencimiento (u$s/tn)`, font: { family: 'Montserrat', size: 12, weight: '600' }, color: '#505845' },
...
function escRenderTabla(c, d) {
  ...
  const rows = deltas.map(dd => {
    ...
    return `<tr class="${Math.abs(dd - d) < 1e-9 ? 'rs-hot' : ''}">
      ...
      <td>${escF(M * (1 + dd))}</td>
      <td>${escF(a)}</td>
      ${hay ? `<td><b>${escF(pr)}</b></td><td class="${escCls(pr - a)}">${escSig(pr - a)}</td><td class="${escCls(pr - a)}">${escUsd((pr - a) * p.disp)}</td>` : ''}
