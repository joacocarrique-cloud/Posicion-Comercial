...
function spCalcSpread(){
  ...
  for(const key of sKeys.slice(0,8)){
    ...
    const isHidden=spExcludedCampaigns.has(key);
    sDatasets.push({label:key,data:pts.map(p=>({x:p.dte,y:p.val})),showLine:true,
      ...
      borderDash:isCurr?[]:[3,3],hidden:isHidden});ci++;
  }
  // ─── Línea promedio (campañas visibles) ───
  const avgByDte={};
  for(const key of sKeys.slice(0,8)){
    if(spExcludedCampaigns.has(key))continue;
  ...
  spChartSeason=new Chart(ctxS,{type:'scatter',data:{datasets:sDatasets},options:{responsive:true,maintainAspectRatio:false,
    plugins:{legend:{display:true,labels:{font:{size:8},boxWidth:10,generateLabels:(chart)=>{
      return chart.data.datasets.map((ds,i)=>{
        const isAvg=ds.label==='Promedio';const isHidden=ds.hidden||false;
        return{text:ds.label,fillStyle:isAvg?'transparent':ds.borderColor,strokeStyle:ds.borderColor,
          lineWidth:isAvg?2.5:ds.borderWidth,lineDash:ds.borderDash||[],hidden:isHidden,
          datasetIndex:i,fontColor:isHidden?'#bbb':undefined,
          textDecoration:isHidden?'line-through':undefined};
      });
    }},onClick:(e,legendItem,legend)=>{
      const ds=legend.chart.data.datasets[legendItem.datasetIndex];
      if(ds.label==='Promedio')return;
      const key=ds.label;
      if(spExcludedCampaigns.has(key))spExcludedCampaigns.delete(key);else spExcludedCampaigns.add(key);
      spCalcSpread();
