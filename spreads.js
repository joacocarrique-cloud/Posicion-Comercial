...
function spCalcSpread(){
  ...
  // Promedio por campaña (equal-weight per campaign)
  const campAvgs={};
  filtered.forEach(s=>{const y=s.year;if(!campAvgs[y])campAvgs[y]={sum:0,n:0};const v=spMode==='basis'?s.basis:s.ratio;if(v!==null){campAvgs[y].sum+=v;campAvgs[y].n++;}});
  const campMeans=Object.values(campAvgs).filter(c=>c.n>0).map(c=>c.sum/c.n);
  const campAvg=campMeans.length>0?campMeans.reduce((a,b)=>a+b,0)/campMeans.length:avg;
  ...
  document.getElementById('sp-stats').innerHTML=`
    ...
    <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:10px;padding:12px;text-align:center;">
      <div style="font-size:9px;font-weight:700;color:var(--text-3);text-transform:uppercase;">Rango Histórico</div>
